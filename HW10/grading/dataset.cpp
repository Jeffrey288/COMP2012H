...
void dataset::resize_image(img_dt& img, const size_t new_rows, const size_t new_cols){
	...
	for (int r = 0; r < new_rows; r++) {
		for (int c = 0; c < new_cols; c++) {
			if ((int) ((double) (r + 0.5) / new_rows * rows) != (int) ((float) (r + 0.5) / new_rows * rows) 
			|| (int) ((double) (c + 0.5) / new_cols * cols) != (int) ((float) (c + 0.5) / new_cols * cols))
				cout << r << " " << c << " "
				<< (int) ((double) (r + 0.5) / new_rows * rows) << " " 
				<< (int) ((float) (r + 0.5) / new_rows * rows) << " "
				<<  (int) ((double) (c + 0.5) / new_cols * cols) << " "
				<<  (int) ((float) (c + 0.5) / new_cols * cols) << " " 
				<<  ((double) (r + 0.5) / new_rows * rows) << " " 
				<<  ((float) (r + 0.5) / new_rows * rows) << " "
				<<   ((double) (c + 0.5) / new_cols * cols) << " "
				<<   ((float) (c + 0.5) / new_cols * cols) << " " << endl;
			new_img[r][c] = img[(int) 
