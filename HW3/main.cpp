...
void print_board() {
    // START YOUR CODES HERE
    int num_length = (num_rows > 10) ? 2 : 1;
...
bool check_whole_board_valid() {
    ...
    int cons[max(MAX_COL, MAX_ROW)]; // observed constraints
...
#define COPY_INTO_BOARD(abacadabra, array) \
    for (int c = 0; c < num_cols; c++) { \
        board[abacadabra][c] = array[c]; \
    }
...
bool _solve(int row_idx) {
    ...
    for (int i = 0; i < num_row_perms[row_idx]; i++) {
        // cout << i;
        COPY_INTO_BOARD(row_idx, row_perms[row_idx][i]);

        // print_board();
        // cout << check_rows_valid(row_idx + 1);
        // cout << check_rows_valid(row_idx + 1);
        // cout << check_rows_valid(row_idx + 1);
        // cout << check_rows_valid(row_idx + 1);
        static bool valid = check_rows_valid(row_idx + 1);
        cout << valid;
        // cout << row_idx << endl;
        if (valid) {
            // cout << "oh hey";
            _solve(row_idx + 1);
            // if (row_idx == num_rows - 1) return true;
            // if (_solve(row_idx + 1)) return true;
            // if (row_idx == num_rows - 1) 
            //     if (check_whole_board_valid())
            //         return true;
            // else if (_solve(row_idx + 1)) return true;
        }
    }
    return false;
