...
QColor* analyzeGuess(const QString& guess, const QString& answer) {
    ...
    for (int i = 0; i < length; i++) {
        if (result[i] != Qt::gray) continue;
        int j = 0;
        for (; j < length; j++) {
            if (guess[i] == answer[j] && !assign[j]) { // matches character but isn't assigned
                break;
            }
        }
        if (j == length) continue; // no match
        // assign it
        result[i] = Qt::yellow;
        assign[j] = true;
    }
...
bool validHardModeGuess(const QString& guess, const bool correctLetters[]) {
    ...
    for (int i = 0; i < NUM_LETTERS; i++) {
        if (correctLetters[i]) {
            if (!guess.contains(QChar((char) (i + 'a')))) {
                return false;
            }
        }
    }
