...
void MultiWordleRound::revertGuess() {
    for (int w=0; w<numWords; w++) {
        if (!correctGuessesMulti[w]) {
            for (int i=0; i<length; ++i) {
                guessesMulti[w][curGuess][i]->setLetter(' ');
            }
        }
    }
    curLetter = 0;
...
void MultiWordleRound::registerEnterKey() {
    ...
    for (int w = 0; w < numWords; w++) {
        if (!correctGuessesMulti[w]) {
            ...
            guess = getCurrentGuess(guessesMulti[w][curGuess], length);

            // Check if guess is in dictionary
            if (!validGuess(guess, dict)) { // actually need to check once only but whatever
                revertGuess();
                emit invalidWord();
                return;
            }
            ...
            colors = analyzeGuess(guess, ansMulti[w]);
            for (int i=0; i<length; ++i) {
                correctGuess &= (colors[i] == GREEN);
                guessesMulti[w][curGuess][i]->setColor(colors[i]);
                emit updateKeyboardColor(guessesMulti[w][curGuess][i]->getLetter(), colors[i]);
            }

            if (!correctGuess) {
                allCorrectGuess = false;
            }
            if (correctGuess) { // sets correctGuessesMulti
                correctGuessesMulti[w] = true;
            }
    ...
    endGuessAttempt(allCorrectGuess);
}
