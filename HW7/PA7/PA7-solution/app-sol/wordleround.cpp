...
QColor* analyzeGuess(const QString& guess, const QString& answer) {
    ...
    QColor* result = new QColor[length];
    bool* assigned = new bool[length];
    // First pass
    // Each correct letter is Green, the rest temporarily color Grey
    ...
    // Second pass
    // For each remaining letter, find if the same letter appears unassigned in the answer elsewhere
    // If yes color it Yellow and assign the answer letter
    for (int i=0; i<length; ++i) {
        if (result[i] == GREEN) continue;
        for (int j=0; j<length; ++j) {
            if (!assigned[j] && guess[i] == answer[j]) {
                result[i] = YELLOW;
                assigned[j] = true;
                break;
            }
...
bool validHardModeGuess(const QString& guess, const bool correctLetters[]) {
    for (int i=0; i<NUM_LETTERS; ++i) {
        if (correctLetters[i] && !guess.contains(QChar('a' + i))) {
            return false;
        }
    }
    return true;
...
void WordleRound::registerEnterKey() {
     ...
     */
    // Hard mode: check if guess uses correct letters
    if (hardMode && !validHardModeGuess(guess, correctLetters)) {
        revertGuess();
     ...
     */
    solver.updateAnswers(guess, colors);
