...
#define REQUIRED_WIDGETS(X) \
    X(LABEL_MODE) \
    X(COMBOBOX_MODE) \
    X(LABEL_WORDLENGTH) \
    X(SPINBOX_WORDLENGTH) \
    X(CHECKBOX_HARDMODE) \
    X(LABEL_NUMWORDS) \
    X(SPINBOX_NUMWORDS) \
    X(LABEL_TIME) \
    X(COMBOBOX_TIME)
#define WIDGET_CHECKS(widget) if (!widget) QFAIL(#widget " not found.");
#define WIDGET_CHECK REQUIRED_WIDGETS(WIDGET_CHECKS)
