    ...
    numWords(numWords)
{
    ...
    for (int i = 0; i < numWords - 1; i++) {
        extraGridLayoutBox[i] = new QGridLayout;
    }

    int numWidgets = 2;
    for (int i = 0; i < numWords - 1; i++) {
        ui->horizontalLayoutGuesses->insertSpacing(numWidgets++, 50);
        ui->horizontalLayoutGuesses->insertLayout(numWidgets++, extraGridLayoutBox[i]);
    }
    ...
    connect(mwr, &WordleRound::updateKeyboardColor, kb, &Keyboard::updateKeyColor);
    connect(mwr, &WordleRound::resetKeyboardColor, kb, &Keyboard::resetKeyColor);
