    ...
    , kb(new Keyboard)
{
     ...
     */
#define CHARACTERS(FUNC) \
    FUNC(A, 'a') FUNC(B, 'b') FUNC(C, 'c') FUNC(D, 'd') FUNC(E, 'e') FUNC(F, 'f') \
    FUNC(G, 'g') FUNC(H, 'h') FUNC(I, 'i') FUNC(J, 'j') FUNC(K, 'k') FUNC(L, 'l') \
    FUNC(M, 'm') FUNC(N, 'n') FUNC(O, 'o') FUNC(P, 'p') FUNC(Q, 'q') FUNC(R, 'r') \
    FUNC(S, 's') FUNC(T, 't') FUNC(U, 'u') FUNC(V, 'v') FUNC(W, 'w') FUNC(X, 'x') \
    FUNC(Y, 'y') FUNC(Z, 'z')
#define ADD_KEYS(chr, lit) kb->addKey(ui->toolButton_##chr, lit);
    CHARACTERS(ADD_KEYS);
...
void RoundWindow::keyPressEvent(QKeyEvent *event) {
    ...
    if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter)
        ...
        kb->delKeyPressed();
    #define ELSE_IFS(chr, lit) else if (event->key() == Qt::Key_##chr) kb->keyPressed(lit);
    CHARACTERS(ELSE_IFS)
}
